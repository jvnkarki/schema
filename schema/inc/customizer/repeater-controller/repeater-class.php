<?php
/**
* Schema customizer repeater class
*
*
* @package Schema
*/
...
if( class_exists('WP_Customize_Control')):
    ...
    */
    class Schema_Repeater_Controler extends WP_Customize_Control {
    	/**
    	 * The control type.
    	 *
    	 * @access public
    	...
    	*/
    	...
    	public $schema_box_label = '';

    	public $schema_box_add_control = '';
    	...
    	public function __construct( $manager, $id, $args = array(), $fields = array() ) {
    		$this->fields = $fields;
    		$this->schema_box_label = $args['schema_box_label'] ;
    		$this->schema_box_add_control = $args['schema_box_add_control'];
    	...
    	public function render_content() {
    		...
    		<ul class="schema-repeater-field-control-wrap">
    			<?php
    			$this->schema_get_fields();
    			?>
    		</ul>

    		<input type="hidden" <?php esc_attr( $this->link() ); ?> class="schema-repeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />
    		<button type="button" class="button schema-add-control-field"><?php echo esc_html( $this->schema_box_add_control ); ?></button>
    		<?php
    	}

    	private function schema_get_fields(){
    		$fields = $this->fields;
    		$values = json_decode($this->value());
    		...
    		?>
    		<li class="schema-repeater-field-control">
    		<h3 class="schema-repeater-field-title"><?php echo esc_html( $this->schema_box_label ); ?></h3>
    		
    		<div class="schema-repeater-fields">
    		<?php
    			foreach ($fields as $key => $field) {
    			$class = isset($field['class']) ? $field['class'] : '';
    			?>
    			<div class="schema-fields schema-type-<?php echo esc_attr($field['type']).' '.esc_attr($class); ?>">
	    			<?php 
	    				...
	    				switch ($field['type']) {
	    					...
	    					case 'upload':
	    						...
	    						echo '<div class="schema-fields-wrap">';
	    						echo '<div class="attachment-media-view">';
	    						echo '<div class="placeholder'.esc_attr($image_class).'">';
	    						esc_html_e('No image selected', 'schema');
	    						echo '</div>';
	    						echo '<div class="thumbnail thumbnail-image">';
	    						echo esc_url($image);
	    						echo '</div>';
	    						echo '<div class="actions clearfix">';
	    						echo '<button type="button" class="button schema-delete-button align-left">'.esc_html__('Remove', 'schema').'</button>';
	    						echo '<button type="button" class="button schema-upload-button alignright">'.esc_html__('Select Image', 'schema').'</button>';
	    					...
	    					case 'category':
	    						...
	    						echo '<option value="0">'.esc_html__('Select Category', 'schema').'</option>';
	    						echo '<option value="-1">'.esc_html__('Latest Posts', 'schema').'</option>';
	    					...
	    					case 'colorpicker':
	    						echo '<input data-default="'.esc_attr($default).'" class="schema-color-picker" data-alpha="true" data-name="'.esc_attr($key).'" type="text" value="'.esc_attr($new_value).'"/>';
	    						break;

	    					case 'icon':
	    						echo '<div class="schema-selected-icon">';
	    						echo '<i class="'.esc_attr($new_value).'"></i>';
	    						echo '<span><i class="fa fa-chevron-down"></i></span>';
	    						echo '</div>';
	    						echo '<ul class="schema-icon-list clearfix">';
	    						$schema_pro_icons_array = sakala_font_icons_array();
	    						foreach ($schema_pro_icons_array as $schema_pro_font_awesome_icon) {
	    							$icon_class = $new_value == $schema_pro_font_awesome_icon ? 'icon-active' : '';
	    							echo '<li class="selected"><i class="fa '. $schema_pro_font_awesome_icon .'"></i></li>';
	    						}
	    						echo '</ul>';
                            ...
                            case 'page':
                                echo '<select data-default="'.esc_attr($default).'"  data-name="'.esc_attr($key).'">';
	    						echo '<option value="0">'.esc_html__('Select Page', 'schema').'</option>';
	    					...
	    					case 'range':
	    						...
	    						echo '<div class="schema-range-slider" >';
	    					...
	    					case 'icon':
	    						...
	    						echo '</div>';
	    						echo '<ul class="schema-icon-list clearfix">';
	    						$schema_icons_array = schema_icons_array();
	    						foreach ($schema_icons_array as $schema_font_awesome_icon) {
	    							$icon_class = $new_value == $schema_font_awesome_icon ? 'icon-active' : '';
	    							echo '<li class='.esc_attr($icon_class).'><i class="fa '.esc_attr($schema_font_awesome_icon).'"></i></li>';
	    						}
	    						echo '</ul>';
	    						echo '<input data-default="'.esc_attr($default).'" type="hidden" value="'.esc_attr($new_value).'" data-name="'.esc_attr($key).'"/>';
	    						break;
	    					...
	    					case 'multicategory':
	    						...
	    						echo '<ul class="schema-multi-category-list">';
	    						echo '<li><label><input type="checkbox" value="-1" '. checked('-1', $new_value, false ) .'/>'.esc_html__( 'Latest Posts', 'schema' ).'</label></li>';
	    						foreach ( $this->cats as $cat ){
    			...
    			<div class="clearfix schema-repeater-footer">
    				<div class="alignright">
    				<a class="schema-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'schema') ?></a> |
    				<a class="schema-repeater-field-close" href="#close"><?php esc_html_e('Close', 'schema') ?></a>
    				</div>
    			</div>
    		</div>
    		</li>
